...
class TrafficSignal:
    ...
    def _diff_queue_length(self):
        total_queue = self.get_total_queued()
        reward = self.last_measure - total_queue
        self.last_measure = total_queue
        return reward
    
    def _mix_diff_queue_length(self):
        change = self._diff_queue_length()
        queue_length = self.get_total_queued()
        reward = change + 1. * (1. / queue_length)

        # self.env.sim_step
        return reward

    def _waiting_time_reward(self):
        reward = -sum(self.get_accumulated_waiting_time_per_lane()) / 100.0
        return reward


    def _sq_diff_waiting_time_reward(self):
        ts_wait = sum(np.square(self.get_accumulated_waiting_time_per_lane())) / 10000.0
    ...
    def _mix_sq_diff_waiting_time_reward(self):
        change = self._sq_diff_waiting_time_reward()
        waiting_time = sum(self.get_accumulated_waiting_time_per_lane()) / 100.0

        w = np.tanh(self.opt_beta * np.maximum(self.env.sim_step - self.opt_warmup_time, 0.01))
        #reward = change - w * waiting_time 
        reward = change - waiting_time
        return reward
    ...
    def _mix_diff_waiting_time(self):
        change = self._diff_waiting_time_reward()
        waiting_time = sum(self.get_accumulated_waiting_time_per_lane()) / 100.0
    ...
    def _mix_diff_waiting_time_recip(self):
        change = self._diff_waiting_time_reward()
        waiting_time = sum(self.get_accumulated_waiting_time_per_lane()) / 100.0

        w = np.tanh(self.opt_beta * np.maximum(self.env.sim_step - self.opt_warmup_time, 0.01))
        reward = change + w * (1. / (waiting_time + 1))

        return reward

    def _symlog(self, x, base = 'e'):
        if base == 'e':
    ...
    def get_per_lane_queued(self) -> List[float]:
        vals = [self.sumo.lane.getLastStepHaltingNumber(lane) for lane in self.lanes]
        return vals
    ...
    reward_fns = {
        ...
        "diff-queue-length": _diff_queue_length,
        "mix-diff-queue-length": _mix_diff_queue_length,
        
        "waiting-time": _waiting_time_reward,
        "diff-waiting-time": _diff_waiting_time_reward,
        "sq-diff-waiting-time": _sq_diff_waiting_time_reward,
        "mix-sq-diff-waiting-time": _mix_sq_diff_waiting_time_reward,
        "mix-diff-waiting-time": _mix_diff_waiting_time,
        "mix-diff-waiting-time-recip": _mix_diff_waiting_time_recip
    }
